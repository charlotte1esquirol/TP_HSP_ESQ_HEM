...
__device__ float* cudaMatrixAdd(float *M1, float *M2, float *Mout, int n, int m){
    int i = threadIdx.x;
    int j = threadIdx.y;
...
__device__ float* cudaMatrixMult3D(float *M1, float *M2, float *Mout, int n, int m, int p){
    int i = threadIdx.x;
    int j = threadIdx.y;
...
__global__ void cudaDense(float* M, float* Mout, float* W, float* B, int M_nb_raw, int M_nb_column, int M_nb_filtre){
    Mout=cudaMatrixMult3D(M, W, Mout, M_nb_raw, M_nb_column, M_nb_filtre);
    Mout=cudaMatrixAdd(Mout, B, Mout, M_nb_raw, M_nb_column);
}
...
// Tests de nos layers sur GPU
    ...
    // Tests sur nos matrices réelles
     ...
     cudaMalloc((void**)&F1_data_g, 1*400*1*sizeof(float));
     cudaMalloc((void**)&W1_kernel_g, 400*120*1*sizeof(float));
     ...
     cudaConv2D<<<gridDim3,blockDim3>>>(S1_data_g, C2_kernel_g, C2_data_g, 14, 14, 5, 16, 10, 10);
     cudaDeviceSynchronize();
     // Application de la fonction d'activation Tanh
     cudaActivation_tanh<<<gridDim3,blockDim3>>>(C2_data_g, 10, 10, 16);
     cudaDeviceSynchronize();
     // Application du sous-échantillonnage
     cudaSubsampling<<<gridDim3,blockDim3>>>(C2_data_g, S2_data_g, 10, 10, 16, 2, 5, 5);
     cudaDeviceSynchronize();
     ...
     cudaDense<<<gridDim2,blockDim2>>>(S2_data_g, D1_data_g, W1_kernel_g, B1_kernel_g, 1, 400, 1);
     cudaDeviceSynchronize();
     //Application de la fonction d'activation Tanh
     cudaActivation_tanh<<<gridDim2,blockDim2>>>(D1_data_g, 1, 120, 1);
     cudaDeviceSynchronize();

     // Application de Dense2
     cudaDense<<<gridDim2,blockDim2>>>(D1_data_g, D2_data_g, W2_kernel_g, B2_kernel_g, 1, 120, 1);
     cudaDeviceSynchronize();
     //Application de la fonction d'activation Tanh
     cudaActivation_tanh<<<gridDim2,blockDim2>>>(D2_data_g, 1, 84, 1);
     cudaDeviceSynchronize();

     // Application de Dense3
     cudaDense<<<gridDim2,blockDim2>>>(D2_data_g, D3_data_g, W3_kernel_g, B3_kernel_g, 1, 84, 1);
     cudaDeviceSynchronize();
     //Application de la fonction d'activation Tanh
     cudaActivation_tanh<<<gridDim2,blockDim2>>>(D3_data_g, 1, 10, 1);
     cudaDeviceSynchronize();
     ...
     cudaMemcpy(C2_data, C2_data_g, 10*10*16*sizeof(float), cudaMemcpyHostToDevice);
     cudaMemcpy(S2_data, S2_data_g, 5*5*16*sizeof(float), cudaMemcpyHostToDevice);
     cudaMemcpy(F1_data, F1_data_g, 1*400*1*sizeof(float), cudaMemcpyHostToDevice);
     cudaMemcpy(D1_data, D1_data_g, 1*120*1*sizeof(float), cudaMemcpyHostToDevice);
     cudaMemcpy(D2_data, D2_data_g, 1*84*1*sizeof(float), cudaMemcpyHostToDevice);
     cudaMemcpy(D3_data, D3_data_g, 1*10*1*sizeof(float), cudaMemcpyHostToDevice);
     cudaDeviceSynchronize();
